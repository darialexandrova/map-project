...
const InteractiveMap = (props: Props): React.JSX.Element => {
	...
	const styles =  useMemo(() => {
		switch (props.mapType) {
			...
			case MapType.AreaByMorbidity: return styleA
		}
	}, [
		props.mapType
	])
	...
	useEffect(() => {
		...
		const reguions = Array.from(areas).filter(a => a.getAttribute('data-code') !== null);
		reguions.forEach(r => r.addEventListener('mouseover', () => {
			const newTitle = r.getAttribute('data-title');
			setRegion(newTitle == null ? '' : newTitle);
		}))
	}, [])
